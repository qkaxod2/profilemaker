        ...
        {records.map((record, index) => (
          <div key={index} role="article" aria-labelledby={`record-title-display-${index}`}>
            ...
            <div className="p-3 border border-slate-300 border-t-0 bg-slate-50 text-slate-800 rounded-b-sm text-sm sm:text-base min-h-[4em]" aria-label={`${record.title} 내용`}>
              {record.content.split('\\n').map((line, i) => <p key={i}>{line}</p>) /* Handle newlines */}
            </div>
          </div>
        ))}
      </div>
...
const ProfileDisplay: React.FC<ProfileDisplayProps> = ({ data }) => {
  return (
    <div className="w-full bg-slate-200 shadow-2xl rounded-lg p-4 sm:p-6 h-full overflow-y-auto" role="document" aria-label="생성된 캐릭터 프로필 미리보기">
      <h1 className="text-2xl sm:text-3xl font-bold text-slate-800 text-center mb-6 sr-only">캐릭터 프로필 시트</h1>
